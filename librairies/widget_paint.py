# -------------------------------------------------------------------------------
# Name:        widget_paint
# Purpose:     widget for texture file
#
...
class Widget_paint(Widget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # space repartition configuration
        ...
        for i in range(w):
            for j in range(h):
                ...
                if color[3] != 0:  # if not transparent pixel
                    ...
                    self.c.create_rectangle(realposx, realposy, realposx + self.scale, realposy + self.scale,
                                            fill=rgba_to_hex(color), width=0)
        ...
        while i < posX:
            ...
            while j < posY:
                ...
                if len(items) > 0:
                    ...
                    draw[i // self.scale, j // self.scale] = hex_to_rgb(color)
    ...
    def __setup(self):
        ...
        self.ishover = False
        self.hover = self.c.create_rectangle(-self.scale * 2, -self.scale * 2, -self.scale, -self.scale,
    ...
    def _resize_canvas(self, event=None):
        ...
        if (width == self.realsize[0] and height == self.realsize[1]) \
                or width <= self.DEFAULT_MIN or height <= self.DEFAULT_MIN:
            return
...
if __name__ == '__main__':
    ...
    fpaint = Widget_paint(root, bg='blue', filepath='../temp/image/temp.png')
    fpaint.grid(row=0, column=0, columnspan=3, sticky='nsew')


    def file_open():
        fpaint.open_file(askopenfilename(title='Open File', filetypes=[("PNG", ".png")]))


    def file_save():
        fpaint.save_file(asksaveasfilename(defaultextension='.png', filetypes=[("PNG", '*.png')], title="Save"))
    ...
    Button(root, text='new', command=fpaint.new_file).grid(row=1, column=2, sticky='nsew')
